...
|---|---|
| `plan_task` | Планирование (`--permission-mode plan`): правка файлов запрещена, но это не песочница — Bash работает по правилам разрешений, при `hooksEnabled` каждая команда ждёт оператора (см. [docs/permissions.md](docs/permissions.md#планирование)). Возвращает план, `session_id` и `plan_digest`. |
